...
function recargar_saldo(owner, valor_recarga, account1, privateKey1) {
    web3.eth.getTransactionCount(account1, (err, txCount) => {
        if (err) {
            console.log(err);
            console.log("Error1, reintentando recargar saldo.........");
            recargar_saldo(owner, valor_recarga, account1, privateKey1);
        }
        //const data =contract.methods.newHash(nombre,cc,correo,fecha).encodeABI()
        ...
        signPromise.then((signedTx) => {
            ...
            sentTx.on("error", err => {
                console.log(err)
                console.log("Error2, firmando la transaccion, reintentando recargar saldo.........");
                recargar_saldo(owner, valor_recarga, account1, privateKey1);
            });
        }).catch((err) => {
            console.log(err);
            console.log("Error3, firmando la transaccion, reintentando recargar saldo.........");
            recargar_saldo(owner, valor_recarga, account1, privateKey1);
        });
